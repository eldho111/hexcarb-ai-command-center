...
const RAW_PANELS: PanelDraft[] = [
  ...
  /* ── SOURCES ─────────────────────────────────────────────────── */
  {
    ...
    quickCalls: withCommonCalls([
      ...
      qc("runtime_status", "Runtime Status", "GET", "/status"),
  ...
  },
  {
    id: "telemetry",
    label: "Live Signals",
    description: "Engine pulse, runtime state, and execution rhythm.",
    section: "diagnostics",
    viewType: "dashboard",
    instructions: "Monitor live engine pulse and runtime state without dropping into missing operations feeds. This view focuses on stable health signals and current system posture.",
    viewConfig: {
      endpoints: [
        { key: "engine_health", endpoint: "/engine/health", label: "Engine Pulse" },
        { key: "status", endpoint: "/status", label: "Runtime Status" },
      ],
      refreshInterval: 15000,
    } satisfies DashboardConfig,
    relatedPanels: [
    ...
    quickCalls: withCommonCalls([
      qc("engine_health", "Engine Pulse", "GET", "/engine/health"),
      qc("runtime_status", "Runtime Status", "GET", "/status"),
      qc("state", "Operational State", "GET", "/state"),
    ]),
  },
  {
    id: "capabilities",
  ...
  },
  {
    id: "engine_repair",
    label: "Engine Triage",
    description: "Guided diagnosis for engine issues using live context.",
    section: "diagnostics",
    viewType: "form-action",
    instructions: "Describe the engine issue in plain language and the assistant will return a triage plan grounded in current model and runtime context.",
    viewConfig: {
      submitEndpoint: "/chat",
      fields: [
        { key: "message", label: "Issue or Request", type: "textarea", required: true, placeholder: "Explain the failure, symptom, or cleanup you want help with." },
        { key: "model_hint", label: "Model Hint", type: "select", options: [
          { value: "fast", label: "Fast" },
          { value: "chat", label: "Balanced" },
          { value: "coder", label: "Coder" },
        ], defaultValue: "fast" },
        { key: "reason", label: "Show reasoning", type: "checkbox", placeholder: "Return the triage reasoning" },
        { key: "verify", label: "Verify references", type: "checkbox", placeholder: "Ask for verification when useful" },
      ],
      submitLabel: "Run Triage",
    } satisfies FormActionConfig,
    relatedPanels: [
      { panelId: "system_status", label: "System Status" },
      { panelId: "telemetry", label: "Live Signals" },
    ],
    quickCalls: withCommonCalls([
      qc("execution_health", "Execution Health", "POST", "/execution/health", { period: "weekly" }),
      qc("triage_chat", "Triage Prompt", "POST", "/chat", { message: "Diagnose the current engine status and propose the next fixes.", model_hint: "fast", reason: true }),
    ]),
  },
  {
    id: "lead_intel",
  ...
  },
  {
    ...
    description: "Model registry, domain inventory, and runtime infrastructure signals.",
    section: "diagnostics",
    viewType: "dashboard",
    instructions: "View the currently registered models, available business domains, and runtime posture without depending on missing storage or ops endpoints.",
    viewConfig: {
      endpoints: [
        { key: "registry", endpoint: "/models/registry", label: "Model Registry" },
        { key: "domains", endpoint: "/domains", label: "Domain Inventory" },
      ],
    } satisfies DashboardConfig,
    relatedPanels: [
      { panelId: "system_status", label: "System Status" },
    ...
    quickCalls: withCommonCalls([
      ...
      qc("domains", "Domain Inventory", "GET", "/domains"),
      qc("status", "Runtime Status", "GET", "/status"),
    ]),
  },
  {
    id: "system_state",
  ...
  },
  {
    id: "lab_items",
    label: "R&D Inventory",
    description: "R&D domain items, lab materials, and experiment support inventory.",
    section: "experiments",
    viewType: "list-detail",
    instructions: "Browse and ingest R&D inventory items using the live R&D domain instead of the missing lab domain.",
    viewConfig: {
      listEndpoint: "/domains/rnd/items",
      listKey: "items",
      idField: "object_id",
      columns: [
      ...
      createEndpoint: "/domains/rnd/ingest",
      createLabel: "Add R&D Item",
      createFields: [
    ...
    quickCalls: withCommonCalls([
      qc("rnd_items", "R&D Items", "GET", "/domains/rnd/items"),
    ]),
  },
  {
    id: "plot",
    label: "Plot Guide",
    description: "Generate plotting prompts and visual-analysis guidance from experiment context.",
    section: "experiments",
    viewType: "form-action",
    instructions: "Ask the assistant how to structure a chart, what variables to compare, and how to explain the resulting visual. This keeps the panel useful until a dedicated plotting service exists.",
    viewConfig: {
      submitEndpoint: "/chat",
      fields: [
        { key: "message", label: "Plot or Analysis Request", type: "textarea", required: true, placeholder: "Suggest the best chart for CNT yield vs temperature and explain how to prepare the data." },
        { key: "model_hint", label: "Model Hint", type: "select", options: [
          { value: "fast", label: "Fast" },
          { value: "chat", label: "Balanced" },
          { value: "coder", label: "Coder" },
      ...
      submitLabel: "Generate Plot Guide",
    } satisfies FormActionConfig,
    relatedPanels: [
      { panelId: "experiment_form", label: "Experiments" },
      { panelId: "measurements", label: "Measurements" },
    ...
    quickCalls: withCommonCalls([
      qc("plot_guidance", "Plot Guidance", "POST", "/chat", { message: "Suggest a visual analysis plan for CNT yield vs temperature from our experiment records.", model_hint: "fast" }),
    ]),
  },
  {
    id: "scout",
  ...
  },
  {
    ...
    description: "High-level company narratives and decision checkpoints.",
    section: "admin",
    viewType: "dashboard",
    instructions: "Review the latest company narrative and recent decisions without depending on the missing oversight endpoints.",
    viewConfig: {
      endpoints: [
        { key: "latest_narrative", endpoint: "/narratives/latest/company", label: "Latest Company Narrative" },
        { key: "decisions", endpoint: "/decisions", label: "Recent Decisions" },
      ],
    } satisfies DashboardConfig,
    relatedPanels: [{ panelId: "approvals", label: "Approvals" }, { panelId: "compliance", label: "Compliance" }],
    quickCalls: withCommonCalls([
      qc("latest_narrative", "Latest Narrative", "GET", "/narratives/latest/company"),
      qc("decisions", "Recent Decisions", "GET", "/decisions"),
    ]),
  },
  {
  ...
  },
  {
    ...
    relatedPanels: [{ panelId: "sales_email_generator", label: "Outreach Studio" }, { panelId: "lead_intel", label: "Lead Intel" }],
    quickCalls: withCommonCalls([qc("domain_sales", "Sales Domain", "GET", "/domains/sales/items")]),
  },
  {
    id: "sales_email_generator", label: "Outreach Studio", description: "Generate and refine sales outreach with the chat engine.", section: "admin",
    viewType: "form-action", instructions: "Describe the audience, offer, and tone you want, and the assistant will generate a draft grounded in the current sales context.",
    viewConfig: {
      submitEndpoint: "/chat",
      fields: [
        { key: "message", label: "Outreach Prompt", type: "textarea", required: true, placeholder: "Draft an outreach email to a thermal battery manufacturer introducing HexCarb's CNT dispersion capabilities." },
        { key: "model_hint", label: "Model Hint", type: "select", options: [
          { value: "fast", label: "Fast" }, { value: "chat", label: "Balanced" }, { value: "coder", label: "Coder" },
        ], defaultValue: "fast" },
        { key: "verify", label: "Ground in knowledge base", type: "checkbox", placeholder: "Ask for evidence-backed language when useful" },
      ],
      submitLabel: "Generate Draft",
    } satisfies FormActionConfig,
    relatedPanels: [{ panelId: "domain_sales", label: "Sales" }],
    quickCalls: withCommonCalls([
      qc("sales_domain", "Sales Domain", "GET", "/domains/sales/items"),
      qc("draft_outreach", "Draft Outreach", "POST", "/chat", { message: "Draft a concise outreach email introducing HexCarb's CNT capabilities to a thermal battery manufacturer.", model_hint: "fast" }),
    ]),
  },
  {
    id: "domain_accounts", label: "Finance", description: "Finance records and operating ledger items.", section: "admin",
    viewType: "list-detail", instructions: "Manage finance-domain records using the live finance domain instead of the missing accounts domain.",
    viewConfig: {
      listEndpoint: "/domains/finance/items", listKey: "items", idField: "object_id",
      columns: [{ key: "object_id", label: "ID", width: "100px" }, { key: "name", label: "Name" }, { key: "type", label: "Type" }],
      createEndpoint: "/domains/finance/ingest", createLabel: "Add Finance Record",
      createFields: [{ key: "payload", label: "Finance Record (JSON)", type: "json", required: true }],
    } satisfies ListDetailConfig,
    relatedPanels: [{ panelId: "compliance", label: "Compliance" }],
    quickCalls: withCommonCalls([qc("finance_items", "Finance Items", "GET", "/domains/finance/items")]),
  },
  {
  ...
  {
    id: "admin_scaffold", label: "Advanced Runner", description: "Low-level diagnostics and raw engine access.", section: "admin",
    viewType: "runner", instructions: "Use the API Runner below for raw engine access, diagnostics, and advanced checks. The quick calls here point only to live endpoints.",
    relatedPanels: [{ panelId: "system_status", label: "System Status" }, { panelId: "telemetry", label: "Live Signals" }],
    quickCalls: withCommonCalls([
      qc("diag_rag", "RAG Diagnostics", "GET", "/diag/rag"),
      qc("state", "System State", "GET", "/state"),
      qc("registry", "Model Registry", "GET", "/models/registry"),
    ]),
  },
];
...
const PANEL_ENHANCEMENTS: Record<string, { compartment: NavCompartmentId; workspaceId?: WorkspaceId; priority?: number; advanced?: boolean; secondaryOnly?: boolean; utility?: UtilityId }> = {
  ...
  domain_accounts: { compartment: "operations", priority: 5 },
